...
const SignupPage = () => {
  ...
  const handleGoogleSignup = async () => {
    ...
    try {
      ...
      dispatch(loadUserCollections(user.uid));
      navigate("/");
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-zinc-950 font-outfit flex flex-col relative overflow-x-hidden">
      {/* Background Glow Orbs */}
      <div className="pointer-events-none absolute inset-0 overflow-hidden -z-0">
        <div
          className="absolute -top-32 -left-20 w-[420px] h-[420px] bg-purple-500/20 dark:bg-purple-600/25 rounded-full blur-3xl animate-pulse"
          style={{ animationDuration: "8s" }}
        />
        <div
          className="absolute top-10 right-0 w-[320px] h-[320px] bg-pink-500/15 dark:bg-pink-600/20 rounded-full blur-3xl animate-pulse"
          style={{ animationDuration: "10s" }}
        />
        <div
          className="absolute bottom-0 left-1/2 -translate-x-1/2 w-[300px] h-[250px] bg-indigo-500/10 dark:bg-indigo-600/15 rounded-full blur-3xl animate-pulse"
      ...
      <main className="flex-1 flex items-center justify-center px-4 py-10 z-10 relative">
        <div className="w-full max-w-[520px]">
          <div className="bg-white/85 dark:bg-zinc-900/85 backdrop-blur-2xl border border-zinc-200/80 dark:border-zinc-800/80 rounded-3xl shadow-2xl shadow-zinc-950/10 dark:shadow-zinc-950/60 p-8 sm:p-11 text-center flex flex-col items-center">
            {/* Google / Brand Icon Badge */}
            ...
            {errorMsg && (
              <div className="w-full mb-5 p-3 rounded-xl bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-900 text-xs text-red-600 dark:text-red-400 text-left">
                {errorMsg}
              </div>
            )}
            ...
            >
              {loading ? (
                <>
                  <svg
                    className="w-5 h-5 animate-spin text-purple-600 dark:text-purple-400"
                    xmlns="http://www.w3.org/2000/svg"
                    fill="none"
                    viewBox="0 0 24 24"
                  >
